...
window.addEventListener("DOMContentLoaded", () => fetchAndRenderBlogs());
...
function fetchAndRenderBlogs(page = 1, searchTerm = "") {
  fetch(BASE_URL)
    ...
    .then(allBlogs => {
      ...
      const paginated = sorted.slice(start, start + PAGE_LIMIT);

      renderBlogs(paginated);
      renderPaginationButtons(Math.ceil(sorted.length / PAGE_LIMIT), page);
    })
  ...
  blogs.forEach(blog => {
    ...
    article.innerHTML = `
      <div class="card-header">
        ...
        <div><p>${blog.author} · ${new Date(blog.date).toDateString()}</p></div>
      </div>
      <div class="card-body">
        <h3>${blog.title}</h3>
